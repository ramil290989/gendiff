  ...
  const nodesWithoutMarkers = keys.map((key) => {
    if (typeof file2Parse[key] !== 'object' && typeof file1Parse[key] !== 'object') {
      if (file2Parse === 0) {
        ...
        return { key, value: file2Parse[key], status: 'nomod' };
      }
    } else {
      if (file2Parse === 0) {
        return { key, value: getNodesWithoutMarkers(file1Parse[key], 0), status: 'nomod' };
      } else {
        return { key, value: getNodesWithoutMarkers(0, file2Parse[key]), status: 'nomod' };
      }
    }
  });
